...
AprilTagNode::AprilTagNode(rclcpp::NodeOptions options)
    : Node("apriltag", "apriltag", options.use_intra_process_comms(true)),
      ...
      impl_(std::make_unique<AprilTagsImpl>()) 
      {
        ...
        param_manager_.addParameter<bool>(invert_colors_, "invert_colors", false);
...
void AprilTagNode::onCameraFrame(
  ...
  // Calcular el valor medio de los píxeles antes de la inversión
  cv::Scalar mean_before = cv::mean(img_rgb8);
  
  // Check if we should invert colors (add this parameter)
  if (invert_colors_) {
    ...
    // Calcular el valor medio después de la inversión
    cv::Scalar mean_after = cv::mean(img_rgb8);
    
    // Mostrar estadísticas con throttle para no inundar el log
    RCLCPP_INFO_THROTTLE(get_logger(), *get_clock(), 5000, 
                       "Color processing active: Original RGB mean: [%.1f, %.1f, %.1f], "
                       "After inversion mean: [%.1f, %.1f, %.1f]",
                       mean_before[0], mean_before[1], mean_before[2],
                       mean_after[0], mean_after[1], mean_after[2]);
  }
