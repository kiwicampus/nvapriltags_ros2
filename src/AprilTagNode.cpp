...
geometry_msgs::msg::Transform ToTransformMsg(const nvAprilTagsID_t& detection)
{
    ...
    const Eigen::Map<const Eigen::Matrix<float, 3, 3, Eigen::ColMajor>> orientation(detection.orientation);
...
struct AprilTagNode::AprilTagsImpl
{
    ...
    void initialize(const AprilTagNode& node, const uint32_t width, const uint32_t height,
...
AprilTagNode::AprilTagNode(rclcpp::NodeOptions options)
    ...
    pub_tf_(create_publisher<tf2_msgs::msg::TFMessage>("/tf", rclcpp::QoS(100)))
    , pub_detections_(create_publisher<nvapriltags_ros2::msg::AprilTagDetectionArray>("detections", rclcpp::QoS(1)))
    , impl_(std::make_unique<AprilTagsImpl>())
{
    ...
    params_callback_handle_ =
        this->add_on_set_parameters_callback(std::bind(&AprilTagNode::parameters_cb, this, std::placeholders::_1));
    ...
    sub_cam_info_ = create_subscription<sensor_msgs::msg::CameraInfo>(
        ...
        std::bind(&AprilTagNode::onCameraInfo, this, std::placeholders::_1), sub_options);

    sub_cam_ = create_subscription<sensor_msgs::msg::Image>(
        "image", rclcpp::QoS(1).best_effort(), std::bind(&AprilTagNode::onCameraFrame, this, std::placeholders::_1),
        sub_options);
    ...
    processing_timer_ = this->create_wall_timer(std::chrono::milliseconds(throttle_interval_ms_),
...
void AprilTagNode::onCameraFrame(const sensor_msgs::msg::Image::ConstSharedPtr& msg_img)
{
    ...
    if (saved_cam_info_)
    {
        ...
        {
            impl_->initialize(*this, img_rgba8.cols, img_rgba8.rows, img_rgba8.total() * img_rgba8.elemSize(),
                              img_rgba8.step, saved_cam_info_);
            RCLCPP_INFO(get_logger(), "Apriltag detection initialized on %s, unsubscribing from %s",
                        sub_cam_->get_topic_name(), sub_cam_info_->get_topic_name());
    ...
    {
        RCLCPP_INFO_THROTTLE(get_logger(), *get_clock(), 20000, "Waiting for camera info on topic %s",
                             sub_cam_->get_topic_name());
    }

    // Copy frame into CUDA buffer
    const cudaError_t cuda_error =
        cudaMemcpy(impl_->input_image_buffer, img_rgba8.ptr(), impl_->input_image_buffer_size, cudaMemcpyHostToDevice);
    if (cuda_error != cudaSuccess)
    {
    ...
    for (int i = 0; i < num_detections; i++)
    {
        ...
        {
            msg_detection.corners.data()[corner_idx].x = detection.corners[corner_idx].x;
            msg_detection.corners.data()[corner_idx].y = detection.corners[corner_idx].y;
        }

        // center
        const float slope_1 =
            (detection.corners[2].y - detection.corners[0].y) / (detection.corners[2].x - detection.corners[0].x);
        const float slope_2 =
            (detection.corners[3].y - detection.corners[1].y) / (detection.corners[3].x - detection.corners[1].x);
        const float intercept_1 = detection.corners[0].y - (slope_1 * detection.corners[0].x);
        const float intercept_2 = detection.corners[3].y - (slope_2 * detection.corners[3].x);
        msg_detection.center.x = (intercept_2 - intercept_1) / (slope_1 - slope_2);
        msg_detection.center.y = (slope_2 * intercept_1 - slope_1 * intercept_2) / (slope_2 - slope_1);

        // Timestamped Pose3 transform
        geometry_msgs::msg::TransformStamped tf;
        tf.header = msg_img->header;
        tf.child_frame_id = std::string(tag_family_) + ":" + std::to_string(detection.id);
                                     ...
                                     std_srvs::srv::SetBool::Response::SharedPtr response)
{
    if (!system_initialized_)
    {
        ...
        RCLCPP_ERROR(this->get_logger(), "Apriltag detector on %s cannot be paused because its not initalized",
                     sub_cam_->get_topic_name());
